...
int net__resolve(const char* hostname, NetFamily family, NetAddress* out_address) {
        ...
        res = getaddrinfo(hostname, NULL, &hints, &res);
        
        if (res != 0) {
            fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(res));
            return -1;
        }
        out_address->family = family;
        if (family == NET_IPV4) {
            struct sockaddr_in* addr4 = (struct sockaddr_in*) res->ai_addr;
            memcpy(out_address->address_ipv4, addr4->sin6_addr, sizeof(out_address->address_ipv4));
        } else if (family == NET_IPV6) {
            struct sockaddr_in6* addr6 = (struct sockaddr_in6*) res->ai_addr;
            memcpy(out_address->address_ipv6, addr6->sin6_addr, sizeof(out_address->address_ipv6));
        }
...
int net__bind(NetConnection* connection, NetProtocol protocol) {

}
void net__close(NetConnection* connection);

// TCP client
int net__connect(NetConnection* connection, NetAddress address);
// TCP server
int net__listen(NetConnection* connection, NetAddress address, NetConnection* out_connection);
// TCP read/write
u64 net__write(NetConnection* connection, void* buffer, u64 size);
u64 net__read(NetConnection* connection, void* buffer, u64 size);

// UDP send/recieve
u64 net__send(NetAddress* address, void* buffer, u64 size);
u64 net__recieve(NetAddress* address, void* buffer, u64 size);
